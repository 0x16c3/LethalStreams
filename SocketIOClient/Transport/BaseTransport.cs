    ...
    public abstract class BaseTransport : IObserver<string>, IObserver<byte[]>, IObservable<IMessage>, IDisposable
    {
        ...
        protected virtual async Task OpenAsync(OpenedMessage msg)
        {
            ...
            if (Options.EIO == 4)
            {
                ...
                {
                    connectMsg.AuthJsonStr = Newtonsoft.Json.JsonConvert.SerializeObject(new[] { Options.Auth }).TrimStart('[').TrimEnd(']');
                }
            }
        ...
        {
            Debug.WriteLine($"[Ping] Interval: {OpenedMessage.PingInterval.ToString()}");
